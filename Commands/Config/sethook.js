      ...
      case "panel":
        {
          ...
          const vcData = client.webhookdb
            .prepare("SELECT * FROM webhookvc WHERE guild_id = ?")
            .get(message.guild.id);
          if (!vcData)
            return message.reply(
              "You need to setup the voice channel first! Use: `.sethook vc channel-id`."
            );
          await channel
            .send({
              embeds: [
            ...
            .then(() => {
              ...
              message.reply(
                ":white_check_mark: Successfully set the hook panel."
              );
            })
            .catch(() =>
              message.reply(
                "Error Occurred! Make sure I have permission to read and send message in " +
                  channel +
                  "."
              )
            );
      ...
      case "vc":
        {
          ...
          if (channel.type !== ChannelType.GuildVoice)
            return message.reply("The channel must be a voice channel.");
          client.webhookdb
            .prepare(
