...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    const selectLang2 = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
    ...
    const selectLang3 = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
    ...
    const sentEmbed = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setColor("#486FFA")
          .setDescription(
            `Select a language to set it as default language of the Bot.\nCurrent Language: ${getlangOutput}`
          )
      ...
      await sentEmbed.edit({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              `<:Tick:1083311747710078976> Successfully Set Language to ${selectedLanguage}`
            )
            .setFooter({
