...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if(!['842620032960823327', '883280307636289567', '712601857473773611'].includes(message.author.id)) return
    ...
    const dbCheck = client.premiumdb.prepare("SELECT * FROM subscriptions WHERE guild_id = ? AND user_id = ?").get(guildID, message.author.id)
    if(!dbCheck) return message.reply(`You're not the booster of the guild: \`${guildName}\``)
    ...
    let description = ""
    client.premiumdb.prepare("DELETE FROM subscriptions WHERE guild_id = ? AND user_id = ?").run(guildID, message.author.id)
