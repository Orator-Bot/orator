...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (subscription) {
      ...
      let description = `:white_check_mark: Added Premium Subscription to Guild ID: \`${guildId}\` with Booster User ID: [${await client.users
        .fetch(userId)
        .then(
          async (u) => await u.tag
        )}](https://discord.com/users/${userId}) for ${ms(ms(time), {
        long: true,
      })}\n`;
      try {
