...
app.post("/setlogging", authenticate, (req, res) => {
  try {
    const { guildId, channelId } = req.body;
    loggingDB.prepare("INSERT OR REPLACE INTO logsdb(guild, channel) VALUES(?,?)").run(guildId, channelId);
    console.log(`Successfully changed logging to ${channelId}`);
    res.status(200).json({ message: `Successfully changed logging to ${channelId}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/resetlogging", authenticate, (req, res) => {
  try {
    ...
    res.status(200).json({ message: `Successfully reset the logging of Guild ID: ${guildId}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/getlogging", authenticate, (req, res) => {
  try {
    const { guildId } = req.body;
    const prefixData = loggingDB.prepare("SELECT * FROM logsdb WHERE guild = ?").get(guildId);
...
app.post("/setvoice", authenticate, (req, res) => {
  try {
    ...
    ttsVoiceDB.prepare("INSERT OR REPLACE INTO customLang(guild, sound) VALUES(?,?)").run(guildId, soundId);
    console.log(`Successfully set the TTS voice to ${soundId}`);
    res.status(200).json({ message: `Successfully set the TTS voice to ${soundId}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/resetvoice", authenticate, (req, res) => {
  try {
    ...
    res.status(200).json({ message: `Successfully reset TTS voice for Guild: ${guildId}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/getvoice", authenticate, (req, res) => {
  try {
    const { guildId } = req.body;
    const ttsVoiceData = ttsVoiceDB.prepare("SELECT * FROM customLang WHERE guild = ?").get(guildId);
...
app.post("/setlanguage", authenticate, (req, res) => {
  try {
    ...
    languageDB.prepare("INSERT OR REPLACE INTO language(guild, lang) VALUES(?,?)").run(guildId, language);
    console.log(`Successfully set the language to ${language}`);
    res.status(200).json({ message: `Successfully set the language to ${language}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/resetlanguage", authenticate, (req, res) => {
  try {
    ...
    res.status(200).json({ message: `Successfully reset language for Guild: ${guildId}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/getlanguage", authenticate, (req, res) => {
  try {
    const { guildId } = req.body;
    const languageData = languageDB.prepare("SELECT * FROM lang WHERE guild = ?").get(guildId);
...
app.post("/setwebhooktext", authenticate, (req, res) => {
  try {
    const { guildId, channelId } = req.body;
    webhookTextDB.prepare("INSERT OR REPLACE INTO webhookchannel(guild_id, channel) VALUES(?,?)").run(guildId, channelId);
    console.log(`Successfully set the webhook text to "${channelId}"`);
    res.status(200).json({ message: `Successfully set the webhook text to "${channelId}"` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/resetwebhooktext", authenticate, (req, res) => {
  try {
    const { guildId } = req.body;
    webhookTextDB.prepare("DELETE FROM webhookchannel WHERE guild_id = ?").run(guildId);
    console.log(`Successfully reset webhook text for Guild: ${guildId}`);
    res.status(200).json({ message: `Successfully reset webhook text for Guild: ${guildId}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/getwebhooktext", authenticate, (req, res) => {
  try {
    const { guildId } = req.body;
    const webhookTextData = webhookTextDB.prepare("SELECT * FROM webhookchannel WHERE guild_id = ?").get(guildId);
...
app.post("/setwebhookvoice", authenticate, (req, res) => {
  try {
    const { guildId, channelId } = req.body;
    webhookVoiceDB.prepare("INSERT OR REPLACE INTO webhookvc(guild_id, channel) VALUES(?,?)").run(guildId, channelId);
    console.log(`Successfully set the webhook voice to channelId: ${channelId}`);
    res.status(200).json({ message: `Successfully set the webhook voice to channelId: ${channelId}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/resetwebhookvoice", authenticate, (req, res) => {
  try {
    const { guildId } = req.body;
    webhookVoiceDB.prepare("DELETE FROM webhookvc WHERE guild_id = ?").run(guildId);
    console.log(`Successfully reset webhook voice for Guild: ${guildId}`);
    res.status(200).json({ message: `Successfully reset webhook voice for Guild: ${guildId}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/getwebhookvoice", authenticate, (req, res) => {
  try {
    const { guildId } = req.body;
    const webhookVoiceData = webhookVoiceDB.prepare("SELECT * FROM webhookvc WHERE guild_id = ?").get(guildId);
    console.log(`Retrieved webhook voice: channelId ${webhookVoiceData?.channel}`);
...
app.post("/setjointocreatevoice", authenticate, (req, res) => {
  try {
    const { guildId, channelId } = req.body;
    joinToCreateVoiceDB.prepare("INSERT OR REPLACE INTO jointocreate(guild, channel) VALUES(?,?)").run(guildId, channelId);
    console.log(`Successfully set the "join to create" voice to "${channelId}"`);
    res.status(200).json({ message: `Successfully set the "join to create" voice to "${channelId}"` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/resetjointocreatevoice", authenticate, (req, res) => {
  try {
    const { guildId } = req.body;
    joinToCreateVoiceDB.prepare("DELETE FROM jointocreate WHERE guild = ?").run(guildId);
    console.log(`Successfully reset "join to create" voice for Guild: ${guildId}`);
    res.status(200).json({ message: `Successfully reset "join to create" voice for Guild: ${guildId}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
app.post("/getjointocreatevoice", authenticate, (req, res) => {
  try {
    const { guildId } = req.body;
    const jointocreateVoiceData = joinToCreateVoiceDB.prepare("SELECT * FROM jointocreate WHERE guild = ?").get(guildId);
    console.log(`Retrieved "join to create" voice: "${jointocreateVoiceData?.channel}"`);
    return res.status(200).json({ message: `${jointocreateVoiceData?.channel}` });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
