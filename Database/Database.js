...
async function loadDatabase(client) {
  ...
  cooldownDB.prepare('CREATE TABLE IF NOT EXISTS cooldowns(command TEXT, user TEXT, timestamp INTEGER, PRIMARY KEY(command, user));').run()
  client.cooldown = cooldownDB.prepare('SELECT timestamp FROM cooldowns WHERE command = ? AND user = ?')
  client.setcooldown = cooldownDB.prepare('INSERT OR REPLACE INTO cooldowns(command, user, timestamp) VALUES(?, ?, ?)')


  // Panel DB
  const panelDB = new client.database('./Database/panel.db')
  panelDB.prepare("CREATE TABLE IF NOT EXISTS panel(guild TEXT PRIMARY KEY, channel TEXT, action TEXT, speaker TEXT)").run()
  client.getpanel = panelDB.prepare("SELECT * FROM panel WHERE guild = ?")
  client.setpanel = panelDB.prepare("INSERT OR REPLACE INTO panel(guild, channel, action, speaker) VALUES(?,?,?,?)")
  client.resetpanel = panelDB.prepare("DELETE FROM panel WHERE guild = ?")

  // Language DB
  const customLang = new client.database("./Database/lang.db")
  customLang.prepare("CREATE TABLE IF NOT EXISTS lang(guild TEXT PRIMARY KEY, lang TEXT)").run()
  client.getlang = customLang.prepare("SELECT * FROM lang WHERE guild = ?")
  client.setlang = customLang.prepare("INSERT OR REPLACE INTO lang(guild, lang) VALUES(?,?)")

  // Orator VC DB
  const oratorvcdb = new client.database("./Database/oratorvc.db")
  oratorvcdb.prepare("CREATE TABLE IF NOT EXISTS oratorvc(guild TEXT PRIMARY KEY, channel TEXT)").run()
  client.setoratorvc = oratorvcdb.prepare("INSERT OR REPLACE INTO oratorvc(guild, channel) VALUES(?,?)")
  client.getoratorvc = oratorvcdb.prepare("SELECT * FROM oratorvc WHERE guild = ?")
  client.resetoratorvc = oratorvcdb.prepare("DELETE FROM oratorvc WHERE guild = ?")

  // Prefix DB
  const prefixdb = new client.database("./Database/prefix.db")
  prefixdb.prepare("CREATE TABLE IF NOT EXISTS prefix(guild TEXT PRIMARY KEY, prefix TEXT)").run()
  client.prefix = prefixdb.prepare("SELECT * FROM prefix WHERE guild = ?")
  client.setprefix = prefixdb.prepare("INSERT OR REPLACE INTO prefix(guild, prefix) VALUES(?,?)")
  ...
  logsdb.prepare("CREATE TABLE IF NOT EXISTS logsdb(guild INTEGER PRIMARY KEY, channel TEXT)").run()
  client.setlogs = logsdb.prepare("INSERT OR REPLACE INTO logsdb(guild, channel) VALUES(?,?);")
  ...
  jtcdatabase.prepare('CREATE TABLE IF NOT EXISTS jointocreate(guild TEXT PRIMARY KEY, channel TEXT)').run()
  client.jtc = jtcdatabase.prepare('SELECT * FROM jointocreate WHERE guild = ?')
  client.setjtc = jtcdatabase.prepare('INSERT OR REPLACE INTO jointocreate(guild, channel) VALUES(?,?)')

  // Soundboard
  const soundboardStateDB = new client.database("./Database/sbstates.db")
  soundboardStateDB.prepare('CREATE TABLE IF NOT EXISTS sbstates(guild TEXT PRIMARY KEY)').run()
  client.sbstate = soundboardStateDB

  // Blacklist Word
  const blacklistwordDB = new client.database('./Database/blacklistword.db')
  blacklistwordDB.prepare('CREATE TABLE IF NOT EXISTS blacklistword(guild_id TEXT, word TEXT, PRIMARY KEY(guild_id, word))').run()
  client.setblacklistword = blacklistwordDB.prepare('INSERT OR IGNORE INTO blacklistword(guild_id, word) VALUES(?,?)')
  client.getblacklistword = blacklistwordDB.prepare('SELECT word FROM blacklistword WHERE guild_id = ?')
  client.resetblacklistword = blacklistwordDB.prepare('DELETE FROM blacklistword WHERE guild_id = ? AND word = ?')

  // Blacklist User
  const blacklistuserDB = new client.database('./Database/blacklistuser.db')
  blacklistuserDB.prepare('CREATE TABLE IF NOT EXISTS blacklistuser(guild_id TEXT, user_id TEXT, PRIMARY KEY(guild_id, user_id))').run()
  client.setblacklistuser = blacklistuserDB.prepare('INSERT OR IGNORE INTO blacklistuser(guild_id, user_id) VALUES(?,?)')
  ...
  client.getblacklistrole = blacklistroleDB.prepare('SELECT * FROM blacklistrole WHERE guild_id = ?')
  client.resetblacklistrole = blacklistroleDB.prepare('DELETE FROM blacklistrole WHERE guild_id = ? AND role_id = ?')
