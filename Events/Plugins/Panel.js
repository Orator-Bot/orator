...
module.exports = {
  ...
  async execute(message, client) {
    ...
    await message.delete()
      .catch(() => {
        message.reply(`Error: Can't delete the message due to **Missing Permissions**.`)
          .then(async (m) => {
            setTimeout(async () => {
              await m.delete()
            }, 3000)
    ...
    let speakerMember = message.member.displayName
    if (panelData.action === 'pause') return message.channel.send('The panel is currently paused.')
    ...
    if (!userVC) return message.channel.send(`You must be in a voice channel.`)
      .then(async (m) => {
        setTimeout(async () => {
          await m.delete()
        }, 5000)
    ...
    if (channelData) {
      if (userVC.id !== channelData.channel) return message.channel.send(`I can only join <#${channelData.channel}>.`)
        .then(async (m) => {
          setTimeout(async () => {
            await m.delete()
          }, 5000)
    ...
    if (botVC) {
      if (userVC !== botVC) return message.channel.send(`${client.user.username} is currently in <#${botVC.id}>.`)
        .then(async (m) => {
          setTimeout(async () => {
            await m.delete()
          }, 5000)
    ...
    await message.channel.send(`[${message.author.tag} said]: ${text}`)
      .then(async (m) => {
        setTimeout(async () => {
          await m.delete()
        }, 5000)
